...
export class TechTypeEffects {
  ...
  saveTechType$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      mergeMap((action) => this.techService.saveTechType(action.techType).pipe(
        map(() => TechTypeActions.saveTechsTypesSuccess()),
        catchError((error) => of(TechTypeActions.saveTechsTypesFailure({error})))
      ))
    );
  });
  ...
  deleteTechType$ = createEffect(() => {
    return this.actions$.pipe(
      ...
      mergeMap((action) => this.techService.deleteTechType(action.techType).pipe(
        map(() => TechTypeActions.deleteTechsTypesSuccess()),
        catchError((error) => of(TechTypeActions.deleteTechsTypesFailure({error})))
      ))
    );
  });
