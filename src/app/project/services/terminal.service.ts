...
export class UserTerminalService {
  ...
  async logInCommand() {
    ...
    const username = await firstValueFrom(
      this.terminal.terminalService.commandHandler
    );

    this.clearCommand();
    this.setPrompt('Contraseña:');

    const inputPrompt = this.terminal.el.nativeElement.children[0].children[1]
  ...
  handleCommand(command: string) {
    ...
    if (action) {
      ...
      this.terminal.terminalService.sendResponse(
        `${command}: comando no encontrado`
      );
    }
    this.stylePrompts();
  }
  ...
  nextCommand() {
    if (
      ...
      this.actualIndex + 1 < this.commandHistory.length
    ) {
      this.terminal.command = this.commandHistory[this.actualIndex + 1];
      this.actualIndex += 1;
  ...
  moveCursorToEnd() {
    const inputSearch = (
      this.terminal.el.nativeElement as HTMLElement
    ).getElementsByClassName('p-terminal-input');
